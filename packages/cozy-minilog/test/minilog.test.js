...
exports['given a minilog'] = {
  ...
  'can pipe into file': function (done) {
    ...
    setTimeout(function () {
      assert.equal(
        fs.readFileSync(__dirname + '/temp.log').toString(),
        'hello world\nfoobar\n'
      )
      done()
    }, 10)
  ...
  'logging an object with a circular structure uses the toString() method of the object':
    function () {
      var circularObject = {
        foo: 'bar'
      }
...
if (module == require.main) {
  var mocha = require('child_process').spawn('../node_modules/.bin/mocha', [
    '--colors',
    '--ui',
    'exports',
    '--reporter',
