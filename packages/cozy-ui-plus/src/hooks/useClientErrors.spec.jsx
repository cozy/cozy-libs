...
jest.mock('../Paywall', () => ({ onClose }) => (
  <>
    QuotaPaywall <button onClick={onClose}>Dismiss</button>
  </>
))
...
xdescribe('useClientErrors', () => {
  ...
  describe('renderErrors', () => {
    ...
    it('displays nothing by default', () => {
      ...
      expect(screen.queryByText('QuotaPaywall')).not.toBeInTheDocument()
    })
    ...
    describe('for quota errors', () => {
      ...
      it('displays a a QuotaPaywall', async () => {
        await setup()

        expect(screen.queryByText('QuotaPaywall')).toBeInTheDocument()
      })
    })
  })
})
