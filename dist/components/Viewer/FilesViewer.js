...
var FilesViewer = function FilesViewer(_ref) {
  ...
  var handleOnChange = function handleOnChange(nextFile) {
    if (onChange) onChange(nextFile.id);
  };

  var currentIndex = files.findIndex(function (f) {
    return f.id === fileId;
  });

  var hasCurrentIndex = function hasCurrentIndex() {
    return currentIndex != -1;
  };

  var viewerFiles = hasCurrentIndex ? files : [currentFile];
