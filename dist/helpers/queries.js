...
var buildFilesQueryByLabels = function buildFilesQueryByLabels(labels) {
  return {
    definition: function definition() {
      return (0, _cozyClient.Q)(_doctypes.FILES_DOCTYPE).where({
        ...
        type: 'file',
        trashed: false
      }).indexFields(['metadata.qualification.label']).select(['name', 'metadata.qualification.label', 'type', 'trashed']).limitBy(1000);
    },
    options: {
